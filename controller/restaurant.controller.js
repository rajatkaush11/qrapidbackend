...
const createRestaurant = async (req, res) => {
  try {
    ...
    const owner = req.user.clientId; // Use clientId as the owner identifier
...
const getRestaurantByUser = async (req, res) => {
  try {
    const restaurant = await RestaurantModel.findOne({ owner: req.user.clientId }); // Use clientId to find the restaurant
    if (!restaurant) {
